...
| 36 | gate-review (R8.2.b), gate-review (§13 P0) | `__tests__/api_key_reveal_modal.test.tsx` | Test gap: assert post-dismiss `onConfirmDismiss` called with no arguments and that the plaintext is not re-emitted. **Sub-gap surfaced during §13 P0 audit**: also missing `Storage.prototype.setItem` spy assertion that the modal never writes the plaintext key to localStorage/sessionStorage. The shaped assertion already exists in `__tests__/feedback_test_form.test.tsx` (the transient-paste surface); mirror it here so both plaintext surfaces are pinned with the same spec. Modal source has no localStorage write path, so the assertion is regression-pin, not bugfix. |
